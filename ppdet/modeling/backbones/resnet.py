...
class ResNet(nn.Layer):
    ...
    def __init__(self,
        ...
        # HARD CODE
        if self.dilation:
            self.res5.res5a.short.conv = nn.Conv2D(1024, 2048, kernel_size=[1, 1], stride=[1, 1], bias_attr=False, weight_attr=ParamAttr(learning_rate=lr_mult_list[-1]))
            self.res5.res5a.branch2b.conv = nn.Conv2D(512, 512, kernel_size=[3, 3], stride=[1, 1], padding=1, bias_attr=False, weight_attr=ParamAttr(learning_rate=lr_mult_list[-1]))
            self.res5.res5b.branch2b.conv = nn.Conv2D(512, 512, kernel_size=[3, 3], stride=[1, 1], padding=2,
                                                      dilation=2, bias_attr=False, weight_attr=ParamAttr(learning_rate=lr_mult_list[-1]))
            self.res5.res5c.branch2b.conv = nn.Conv2D(512, 512, kernel_size=[3, 3], stride=[1, 1], padding=2,
                                                      dilation=2, bias_attr=False, weight_attr=ParamAttr(learning_rate=lr_mult_list[-1]))
    ...
    def forward(self, inputs):
        x = inputs['image']
        conv1 = self.conv1(x)
        x = F.max_pool2d(conv1, kernel_size=3, stride=2, padding=1)  # OK
        outs = []
