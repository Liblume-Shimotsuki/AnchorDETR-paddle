...
class AnchorDETRHead(nn.Layer):
    ...
    def __init__(self,
                 ...
                 with_mask_head=False,
                 use_focal_loss=False):
        super(AnchorDETRHead, self).__init__()
        # add background class
        self.num_classes = num_classes if use_focal_loss else num_classes + 1
        self.hidden_dim = hidden_dim
        self.loss = loss
        ...
        self.bbox_embed = MLP(hidden_dim,
                              hidden_dim,
                              output_dim=4,
                              num_layers=num_mlp_layers)
        self._reset_parameters()
    ...
    def _reset_parameters(self):
        linear_init_(self.class_embed)
        num_classes = self.num_classes
        prior_prob = 0.01
        bias_value = -math.log((1 - prior_prob) / prior_prob)
        self.class_embed.bias.set_value(paddle.ones([num_classes]) * bias_value)

        constant_(self.bbox_embed.layers[-1].weight, 0.0)
        self.bbox_embed.layers[-1].bias.set_value(paddle.to_tensor([0, 0, -2.0, -2.0]))
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        ...
        Args:
            ...
            inputs (dict): dict(inputs)
        """
        reference, feats = out_transformer["reference"], out_transformer["output"]
